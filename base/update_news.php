...
<body data-type="widgets">
    ...
    <script type="text/javascript">
      ...
      $.get('/api/public/news_id.php?newsid=' + getQueryString('id'), function (data) {
        ...
        $('#img_container').val(data.data.imgurl);
        editor.$txt.html(data.data.content);

        //初始化select组件
        $('#type').selected('destroy');
        $('#type').val(data.data.type);
        // 把 Amaze UI selected 重新赋值给 $.fn.amSelected
      ...
      $('.tpl-btn-bg-color-success').on('click', function () {
        ...
        $.post('/base/api/update_news.php', obj, function (data) {
          if(data.status){
            (new Alert('提示', '文章更新成功', function () {
              location.reload();
            })).show();
          }else{
            (new Alert('提示', data.info)).show();
