<?php
  ...
  $query_stmt = $pdo->prepare('SELECT email, userid from  admin WHERE email=:email AND password=:password;');
  ...
  //查询到了数据，写入session
  $_SESSION['_admin_userid'] = $row->userid;
  $_SESSION['_admin_email'] = $row->email;
