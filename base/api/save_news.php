<?php
  ...
  //插入数据
  $sql = <<<EOF
    INSERT news(newsid, userid, title, author, type, time, content, keywords, imgurl) 
    VALUES(:newsid, :userid, :title, :author, :type, :time, :content, :keywords, :imgurl)
EOF;
  ...
  if($stmt->execute() && $stmt->rowCount()){
    $stmt = null;
    Utils::json(1, '文章创建成功', 'article create success');
  }else{

    $stmt = null;
    Utils::json(0, '文章创建失败', 'insert db error');
  }
